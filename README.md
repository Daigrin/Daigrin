...
- **Updates** (`updates`) — `auto_update` on a 45-minute check interval, `verify_signatures` so only signed updates are applied, and `rollback_on_failure` restoring the previous version if an update fails; threat intelligence from centralized servers, cloud services, peer-to-peer networks, and local sources; delivered over HTTPS, SFTP, or SSH (ftp removed because it sends credentials in plaintext); as executables, scripts, configs, or database updates.
